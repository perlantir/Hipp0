...
A detailed comparison of Hipp0 against other AI memory systems: Mem0, MemPalace, Zep, and LangMem. For the summary table, see the [README](../README.md#how-hipp0-compares).
...
| Aspect | Hipp0 | Mem0 / LangMem / MemPalace | Zep |
|--------|-------|----------------------------|-----|
...
Zep uses two retrieval signals (graph relationships and embedding similarity), which is more than pure embedding systems but fewer than Hipp0's five.
...
| **MemPalace** | None | Single-user memory system. |
...
|--------|--------|-------------|---------|
| **Hipp0** | H0C (Hipp0 Compressed) | 3.4x (benchmark), 10-12x (production) | Custom binary-inspired format. Encodes decisions with single-char field markers, pipe delimiters, severity flags. Lossless for decision content. See [H0C format spec](h0c-format.md). |
| **MemPalace** | AAAK | Varies | Alternative compression format. |
...
vs equivalent JSON at ~4x the token count.
...
| **Mem0** | ⚠️ Open-source available | Open-source version available (Apache 2.0) with `pip install mem0ai`. Cloud platform at app.mem0.ai is the primary offering with additional enterprise features. |
| **Zep** | ⚠️ Community Edition deprecated | Zep Cloud is the primary product. The open-source Community Edition exists in the repo but is no longer actively maintained. Graphiti (the underlying graph framework) remains open source. |
| **LangMem** | ✅ Self-hosted | Runs against your own PostgreSQL or other storage backends. MIT license. |
| **MemPalace** | ❌ Cloud only | Proprietary cloud service. |
...
- You prefer a managed cloud service with enterprise compliance (SOC2/HIPAA)
...
- MemPalace — proprietary, limited public documentation
