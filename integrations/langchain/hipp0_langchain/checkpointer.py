"""
hipp0-langchain — LangGraph Checkpointer
=========================================
A LangGraph ``BaseCheckpointSaver`` implementation that stores checkpoints as
Hipp0 session summaries and rehydrates state by compiling context from Hipp0.

This lets LangGraph persist agent state across runs without any external
key-value store — Hipp0 itself acts as the durable state layer.
...
try:
    ...
    raise ImportError(
        "langchain-core is required for hipp0-langchain. "
...
class Hipp0Checkpointer(BaseCheckpointSaver):
    ...
    and saves it as a Hipp0 ``SessionSummary``.  Each ``get`` operation
    ...
    project_id:
        The Hipp0 project to store checkpoints under.
    agent_name:
    ...
    def get_tuple(self, config: RunnableConfig) -> CheckpointTuple | None:
        ...
        Hipp0 session summaries are queried by agent name; the latest one
    ...
    def put(
        ...
        try:
            ...
            logger.debug(
                "Hipp0Checkpointer.put: saved checkpoint %s as session %s",
                checkpoint_id,
                session.get("id"),
            )
        except Hipp0Error as exc:
    ...
    def _session_to_tuple(
        ...
        """Deserialise a Hipp0 session summary back into a CheckpointTuple."""
        meta = session.get("metadata", {})
        ...
        try:
            ...
            logger.warning("Hipp0Checkpointer: failed to parse checkpoint payload — %s", exc)
            return None
