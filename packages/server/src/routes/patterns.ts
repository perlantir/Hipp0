...
import { getProjectPatterns, extractPatterns } from '@decigraph/core/intelligence/pattern-extractor.js';
...
export function registerPatternRoutes(app: Hono): void {
  ...
  app.get('/api/projects/:id/patterns', async (c) => {
    ...
    try {
      ...
      console.warn('[decigraph:patterns] Failed to get patterns:', (err as Error).message);
      return c.json([]);
    }
  });
  ...
  app.post('/api/patterns/extract', async (c) => {
    try {
      ...
      console.error('[decigraph:patterns] Extraction failed:', (err as Error).message);
