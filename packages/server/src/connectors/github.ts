...
async function postRelevantDecisionsComment(
  ...
  try {
    ...
    console.warn('[decigraph/github] Failed to list PR files:', (err as Error).message);
    return;
  }
  ...
  // Build comment body
  const dashboardUrl = process.env.DECIGRAPH_DASHBOARD_URL ?? 'http://localhost:3200';
  const lines = [
    '### DeciGraph — Relevant Decisions\n',
  ...
  lines.push('\n*Auto-generated by DeciGraph*');

  const commentBody = lines.join('\n');

  // Check for existing DeciGraph comment (update, don't duplicate)
  try {
    const comments = await octokit.issues.listComments({ owner, repo, issue_number: prNumber, per_page: 100 });
    const existing = comments.data.find((c) =>
      c.body?.includes('### DeciGraph — Relevant Decisions'),
    );
    ...
    console.warn('[decigraph/github] Failed to post comment:', (err as Error).message);
  }
...
export async function notifySupersededDecision(
  ...
  for (const row of links.rows as Record<string, unknown>[]) {
    ...
    try {
      ...
      console.warn(`[decigraph/github] Failed to notify PR ${externalId}:`, (err as Error).message);
    }
  }
}
  ...
  app.post('/api/webhooks/github', async (c) => {
    ...
    if (payload.action === 'opened' || payload.action === 'edited') {
      ...
      postRelevantDecisionsComment(repoFullName, prNumber, prUrl, prTitle, prAuthor, projectId).catch(
        (err) => console.warn('[decigraph/github] Comment posting failed:', (err as Error).message),
      );

      console.log(`[decigraph/github] PR #${prNumber} ${payload.action} — ${linkCount} reference(s) linked`);
    ...
    if (payload.action === 'closed' && pr.merged) {
      ...
      console.log(`[decigraph/github] PR #${prNumber} merged — decision extraction queued (by ${madeBy})`);
